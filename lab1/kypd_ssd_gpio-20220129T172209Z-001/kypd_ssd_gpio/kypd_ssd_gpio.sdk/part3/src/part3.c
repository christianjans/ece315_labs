	   ...
	   while (1) {
	      ...
	      if (status == KYPD_SINGLE_KEY
	         ...
	         if((char)key == 'F'){
	        	 ...
	        	 /*******************************/
	        	 xQueueSendToBack(xQueue, &current_value, 0U);
	        	 xil_printf("Added %d to the queue\n",current_value);
	         ...
	         //e.g. if the user presses the following digits in this order 4 > 5 > 8  => current_value will end up being 458
	        	 ...
	        	 /*****************************************/
	        	 xQueueSendToBack(xQueue, &key, 0U);
...
static void prvRxTask( void *pvParameters )
{
	...
	for( ;; )
	{
		...
		xQueueReceive(xQueue, &store_operands[2], 0U);

		switch((char)store_operands[2]) {

		case 'A': {
			//add
			if (store_operands[0] > INT_MAX - store_operands[1]) xil_printf("Error: Overflow\n");
			else xil_printf("%d + %d = %d\n", store_operands[0], store_operands[1], store_operands[0] + store_operands[1]);
			break;
		}
		case 'B': {
			//subtract
			if (store_operands[1] > store_operands[0]) xil_printf("Error: Underflow\n");
			else xil_printf("%d - %d = %d\n", store_operands[0], store_operands[1], store_operands[0] - store_operands[1]);
			break;
		}
		case 'C': {
