        ...
        if (status == KYPD_SINGLE_KEY
         ...
         //if user presses 'E' key, consider the last input key pressed as the operand
           ...
           if(store_key != 'x'){
             ...
             int temp = (int) store_key - 48;
             xQueueSendToBack(xQueue, &temp, 0U);
             /****************************************/
          }
         }
        }
...
//      xil_printf("op0 = %d\n\n",store_operands[0]);
//      xil_printf("op1 = %d\n\n",store_operands[1]);
      if (store_operands[1] == 0) {result = -1;}
      else {result = store_operands[0] % store_operands[1];}
      break;
